...
def crossValidationDT():
    ...
    for train_index_o, test_index_o in CV1.split(X,y):
        ...
        model2 = tree.DecisionTreeClassifier(max_depth=reg_term,criterion = "entropy") ###NEED REGU "gini"
        model2 = model2.fit(X_train_o, y_train_o)
        y_dectree = model2.predict(X_test_o)
        current_err = 100*(y_dectree!=y_test_o).sum().astype(float)/len(y_test_o)
    ...
    print("Decision Tree generalization error: %f with %s and %i" % (e_gen,'max depth',mode_reg[0]))
